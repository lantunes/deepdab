...
class TDOneGradient3x3PolicyCNNV2(Policy):
    """
    Adds padding to the initial convolutional layer.
    """
    def __init__(self, board_size):
        ...
        self._sum_grad_W_in = tf.placeholder("float", shape=[7 * 7 * 12, self._n_hidden], name="sum_grad_W_in")
        ...
        self._W_in = tf.Variable(tf.random_normal([7 * 7 * 12, self._n_hidden], 0.0, 0.1), name="W_in")
        ...
        # Input Tensor Shape (for the 3x3 board): [1, 7, 7, 1] (batch size, width, height, channels)
        # Output Tensor Shape: [1, 7, 7, 12]
        ...
        self._conv_flat = tf.reshape(self._conv, [1, 7 * 7 * 12])
    ...
    def get_architecture(self):
        return "7x7-conv(3x3, relu, 12)-tanh(500)-sigmoid(1)"
