...
class PGPolicyCNN2b(Policy):
    """
    Adds a second convolutional layer. Expanded edge matrix
    """
    def __init__(self, board_size, batch_size=1, existing_params=None, dropout_keep_prob=1.0, activation=tf.nn.relu):
        ...
        with g.as_default():
            ...
            # Input Tensor Shape (for the 2x2 board): [None, 7, 7, 1] (batch size, width, height, channels)
            ...
            # Computes 24 features using a 3x3 filter with some activation.
            # No padding is added.
    ...
    def get_architecture(self):
        return "7x7-conv(3x3, %s, 12)-conv(3x3, %s, 24)-tanh(300)-softmax(1)" % \
