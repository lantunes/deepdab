...
class MCTSPolicyNetPolicy:
    def __init__(self, board_size, num_playouts, c=sqrt(2), w=10, default_policy=None):
    ...
    def select_edge(self, board_state, root_player_score, policy_net):
        root_node = _Node(board_state, self._c, self._w)
        ...
        for i in range(self._num_playouts):
            ...
            if node.has_untried_moves():
                ...
                current_player = game.select_edge(selected_index, current_player)
                node = node.add_child(move_state, self._c, self._w)
...
class _Node:
    def __init__(self, state, c, w, parent=None):
        self.state = state
        self._c = c
        self._w = w
        self.wins = 0
    ...
    def add_child(self, child_state, c, w):
        child = _Node(child_state, c, w, parent=self)
    ...
    def ucb1(self):
        ...
        return self.wins/self.visits + self._c*sqrt(log(self.parent.visits)/self.visits) + self._w*(self.prob/(self.visits + 1))
