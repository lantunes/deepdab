...
# we're using MCTS self-play, where the search tree is persistent from episode to episode;
#  the number of tree nodes won't grow, as each state is only stored once, but the number of visits and wins in each
#  node will grow over the course training, providing a better estimate of the value of states;
# the number of playouts for each move aren't too important, as the search tree is persistent
mcts_player = MCTSPolicy(board_size, num_playouts=100)
reward_fn = DelayedBinaryReward()

print_info(board_size=board_size, num_episodes=num_episodes, policy=policy, mode='MCTS self-play', reward=reward_fn,
