...
class PGPolicyCNN4(Policy):
    ...
    def __init__(self, board_size, batch_size=1, existing_params=None, dropout_keep_prob=1.0, activation=tf.nn.relu):
        ...
        with g.as_default():
            ...
            self._W_in = tf.get_variable(shape=[3 * 3 * 12, self._n_hidden], initializer=W_in_initializer, name="W_in")
            ...
            # Padding is added to preserve width and height.
            # Input Tensor Shape (for the 2x2 board): [None, 5, 5, 48] (batch size, width, height, channels)
            ...
            # Padding is added to preserve width and height.
            # Input Tensor Shape (for the 2x2 board): [None, 5, 5, 36] (batch size, width, height, channels)
            ...
            # No padding is added.
            # Input Tensor Shape: [None, 5, 5, 24]
            # Output Tensor Shape: [None, 3, 3, 12]
            ...
            self._conv_flat = tf.reshape(self._conv2, [tf.shape(self._input)[0], 3 * 3 * 12])
    ...
    def get_architecture(self):
        return "5x5-conv(3x3, %s, 48)-conv(3x3, %s, 36)-conv(3x3, %s, 24)-conv(3x3, %s, 12)-tanh(300)-softmax(1)" % \
