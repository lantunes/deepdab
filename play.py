...
import deepdab
import deepdab.ai
from deepdab.util.initializer_util import read_params
...
def main():
    ...
    L1 = deepdab.ai.Level1HeuristicPolicy((board_rows, board_cols))
    L2 = deepdab.ai.Level2HeuristicPolicy((board_rows, board_cols))

    CEP = deepdab.ai.CausalEntropicPolicy((board_rows, board_cols), max_sample_paths=1000)

    if board_rows == 2 and board_cols == 2:
        td0 = deepdab.ai.TDZeroPolicy((board_rows, board_cols), learning_rate=0.0, gamma=0.0,
                                    table_file_path='resources/td0_2x2_0.28_0.01_0.99_immediate_reward.txt')

        td1 = deepdab.ai.TDOneTabularPolicy((board_rows, board_cols), learning_rate=0.0, gamma=0.0,
        ...
        pg_model = deepdab.ai.PGPolicyCNN2((board_rows, board_cols), existing_params=pg_params, activation=tf.nn.tanh)
        ...
        MCTS_PG = deepdab.ai.MCTSPolicy2((board_rows, board_cols), num_playouts=10000, default_policy=pg_model)
    else:
        td0 = None
        td1 = None
        pg_model = None
    ...
    if board_rows == 3 and board_cols == 3:
        ...
        OPP = deepdab.ai.PGPolicy3x3CNN((board_rows, board_cols), existing_params=pg_params3, activation=tf.nn.relu)
        OPP.set_boltzmann_action(False)
        OPP.set_epsilon(0.0)
        MCTS_PG2 = deepdab.ai.MCTSPolicyNetPolicyCpuct((board_rows, board_cols), num_playouts=1000, cpuct=5,
                                               normalize_policy_probs_with_softmax=False)
    else:
        OPP = None
        MCTS_PG2 = None

    game = deepdab.Game((board_rows, board_cols), players)
    print(game)
    while not game.is_finished():
        current_player = game.get_current_player()

        if current_player == "$L0":
            move = deepdab.RandomPolicy().select_edge(game.get_board_state())
